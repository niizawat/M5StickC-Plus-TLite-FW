...
def validate_temperature_data(temperature_data: List[float]) -> None:
    ...
    if len(temperature_data) != 384:  # 16x24
        raise ValueError(f"温度データの要素数が無効です: {len(temperature_data)} (384要素である必要があります)")
    if not all(isinstance(x, (int, float)) for x in temperature_data):
        raise ValueError("温度データに数値以外の要素が含まれています")

def create_thermal_image(temperature_data: List[float], width: int = 16, height: int = 24) -> io.BytesIO:
    """
    温度データからサーマルイメージを生成する
    
    Args:
        temperature_data (List[float]): 温度データのリスト（384要素）
        width (int): 画像の幅（デフォルト: 16）
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # クラウド接続機能からのデータ形式に対応
        device_id = body.get('macaddr')  # MACアドレスをデバイスIDとして使用
        temperature_data = body.get('frame')  # 温度データ配列
        
        if not device_id:
            raise ValueError("device_idが指定されていません")
        if not isinstance(device_id, str):
            raise ValueError("device_idは文字列である必要があります")
        ...
        # デバイスから送信された統計値を使用
        stats = {
            'center_temp': float(body.get('center', 0.0)),
            'avg_temp': float(body.get('average', 0.0)),
            'max_temp': float(body.get('highest', 0.0)),
            'min_temp': float(body.get('lowest', 0.0)),
        ...
        key = f'thermal_images/{device_id}/{timestamp_str}.png'
        
        metadata = {
            'device_id': device_id,
            'timestamp': timestamp_str,
            'datetime': body.get('datetime', ''),  # 元のタイムスタンプを保存
            'interval': str(body.get('interval', '')),  # アップロード間隔を保存
